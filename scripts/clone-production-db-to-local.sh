...
if [[ -z "${SOURCE_DB_URL:-}" ]]; then
  echo "ERROR: SOURCE_DB_URL is required (production PostgreSQL connection string)"
  echo "Usage: SOURCE_DB_URL='postgresql://...' $0"
  exit 1
fi

TARGET_DB_PORT="${TARGET_DB_PORT:-${GITPINS_DB_PORT:-5432}}"
TARGET_ADMIN_URL="${TARGET_ADMIN_URL:-postgresql://gitpins:gitpins@localhost:${TARGET_DB_PORT}/postgres}"
TARGET_DB_URL="${TARGET_DB_URL:-postgresql://gitpins:gitpins@localhost:${TARGET_DB_PORT}/gitpins_local_clone}"
TARGET_DB_NAME="${TARGET_DB_NAME:-gitpins_local_clone}"
DUMP_FILE="${DUMP_FILE:-/tmp/gitpins-production-$(date +%s).dump}"
